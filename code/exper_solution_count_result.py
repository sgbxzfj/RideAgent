...
def _summarize_baseline_by_index(
    baseline_df: pd.DataFrame,
    output_csv: str = "satisfaction_solution_count_index_summary.csv",
) -> pd.DataFrame | None:
    """
...
if __name__ == '__main__':
    ...
    result_all_df.to_csv('satisfaction_solution_count.csv', index=False, encoding="utf_8_sig")
    ...
    if insample_summary_df is not None:
        insample_summary_df.to_csv('satisfaction_solution_count_insample_summary.csv', index=False, encoding="utf_8_sig")
    if outsample_summary_df is not None:
        outsample_summary_df.to_csv('satisfaction_solution_count_outsample_summary.csv', index=False, encoding="utf_8_sig")
    ...
    if pattern_tables or insample_summary_df is not None or outsample_summary_df is not None:
        ...
        with pd.ExcelWriter('satisfaction_solution_count.xlsx', engine='xlsxwriter') as writer:
            if pattern_tables:
