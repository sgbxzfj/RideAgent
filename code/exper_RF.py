...
def process_string_and_add_constraints(input_string,model):
    ...
    global pattern
    csv_directory = f'/Users/admin/Documents/原文稿/jupyter/AAAI/experiment0720/p_split_{pattern}'
...
def expand_range_in_brackets(s):
    def replace_range(match):
        full = match.group(0)      # e.g., "[1-72]"
        content = match.group(1)   # e.g., "1-72"
        if '-' in content:
            start, end = map(int, content.split('-'))
            expanded = ','.join(str(i) for i in range(start, end + 1))
            return f'[{expanded}]'
        else:
            return full  # already like [5]
    
    # Match patterns like [1-72], [3-5], [10-10], but not [1,2,3]
    return re.sub(r'\[(\d+-\d+)\]', replace_range, s)

def process_string_and_add_constraints(input_string, model, csv_directory):
    # Path to the directory containing CSV files
    # csv_directory = '/Users/admin/Documents/原文稿/jupyter/AAAI/experiment0720/split'
    ...
    cleaned_string = expand_range_in_brackets(cleaned_string)
...
def sampler(user_target_indicator:list) -> list:
    ...
    global scope_down, cluster_num, cluster_per_table, reward_num, reward_per_table, db_name
    database = SQLDatabase.from_uri(f"mysql+pymysql://root:Zhy15810886562!@127.0.0.1/{db_name}")  
...
if __name__ == "__main__":
    ...
    global Index, db_name
    nt = 150

    db_name = f'r_vehicle_{nt}'
    randseed = 0  # 随机种子
    dv_num = 1032
    
    ###!!!personalized!!!###
    locality = 0.4
    Index = 'Improve user satisfaction of vehicles'
    ...
    gurobi_model_path = f'/Users/admin/Documents/原文稿/jupyter/AAAI/experiment0720/RF_train/MIP_model/model_base_nt{nt}_depth150.mps'
    # csv_path = f'/Users/admin/Documents/原文稿/jupyter/AAAI/experiment0720/Demand_pattern/demand_pattern/total_data_{pattern}.csv'
    subdata_csv_path = f'/Users/admin/Documents/原文稿/jupyter/AAAI/experiment0720/r_split_{nt}'
    supply_num = 42
    ...
    ###!!!personalized!!!###
        ...
        # temp_text = """model.setObjective(gp.quicksum(I[i][k] - gp.quicksum(model.getVarByName(f'cluster{i}_cluster{j}_{k}') for j in D) for i in S for k in K), GRB.MINIMIZE)"""
        # Gurobi_formulation = temp_text
        ...
        # scope_down = """「cluster0:[1,2,3],cluster1:[4,5,6],cluster2:[7,8,9],cluster3:[10,11,12],cluster5:[13,14,15],cluster6:[16,17,18],cluster8:[19,20,21],cluster9:[22,23,24],reward12:[25,26,27],reward15:[28,29,30],reward37:[31,32,33],reward38:[34,35,36],reward4:[37,38,39],reward44:[40,41,42],reward49:[43,44,45],reward7:[46,47,48]」"""
        print(type(scope_down), scope_down)
        try:
        ...
        tnow = time.time()
        if In_sample:
            file_path = '/Users/admin/Documents/原文稿/jupyter/AAAI/exper_rf/{}/insample/index{}_sample{}_{}.txt'.format(nt, Index, selected_num, tnow)
        else:
            file_path = '/Users/admin/Documents/原文稿/jupyter/AAAI/exper_rf/{}/outsample/index{}_sample{}_{}.txt'.format(nt, Index, selected_num, tnow)
        f = open(file_path, 'w')
