...
PATTERN_DATA_DIR = os.path.join('data', 'demand_pattern')
DEFAULT_PATTERN = 'base'
CSV_PATH_TEMPLATE = os.path.join(PATTERN_DATA_DIR, 'total_data_{}.csv')
LEGACY_TOTAL_DATA = os.path.join('data', 'total_data.csv')
...
def _summarize_baseline_sample_bins(
    ...
    output_csv: str = "satisfaction_sample_bins_summary.csv",
) -> None:
    """
...
def _summarize_baseline_by_index(
    baseline_df: pd.DataFrame,
    output_csv: str = "satisfaction_sample_index_summary.csv",
) -> pd.DataFrame | None:
    """
...
if __name__ == '__main__':
    ...
    result_all_df.to_csv('satisfaction_pattern.csv', index=False, encoding="utf_8_sig")

    # 追加 baseline（base 场景）结果
    baseline_csv = 'satisfaction_sample.csv'
    ...
    if insample_summary_df is not None:
        insample_summary_df.to_csv('satisfaction_pattern_insample_summary.csv', index=False, encoding="utf_8_sig")
    if outsample_summary_df is not None:
        outsample_summary_df.to_csv('satisfaction_pattern_outsample_summary.csv', index=False, encoding="utf_8_sig")
    ...
    if pattern_tables or insample_summary_df is not None or outsample_summary_df is not None:
        ...
        with pd.ExcelWriter('satisfaction_pattern_by_scenario.xlsx', engine='xlsxwriter') as writer:
            if pattern_tables:
